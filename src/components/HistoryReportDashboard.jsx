...
export default function HistoryReportDashboard({ history = [] }) {
  ...
  const normalized = useMemo(() => {
    return history
      .map((h) => {
        const start = h.startTime ? new Date(h.startTime) : h._start || null;
        const end = h.endTime ? new Date(h.endTime) : h._end || null;
        const finishedAt = h.finishedAt ? new Date(h.finishedAt) : h._finishedAt || null;
        return { ...h, start, end, finishedAt };
      })
      .filter((i) => i.finishedAt && i.finishedAt.toISOString().startsWith(todayStr) && i.start && i.end);
  }, [history, todayStr]);
  ...
  const shiftData = useMemo(() => {
    ...
    normalized.forEach((entry) => {
      const finish = entry.finishedAt;
      const hour = finish.getHours();
      const minute = finish.getMinutes();
      const total = parseInt(entry.totalPrice || entry.total || 0, 10) || 0;
      const isPagi = (hour > 9 && hour < 16) || (hour === 16 && minute < 45);
      const isMalam = (hour > 16 || (hour === 16 && minute >= 45)) && hour <= 23;
      if (isPagi) pagi += total;
      else if (isMalam) malam += total;
    ...
    <div className="flex flex-col gap-6 text-gray-100">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-r from-green-700/10 to-emerald-500/6 border border-green-500/20 rounded-xl p-5">
          ...
          <h2 className="text-2xl font-bold text-green-400 mt-1">{formatCurrency(shiftData.pagi)}</h2>
        </div>
        ...
        <div className="bg-gradient-to-r from-indigo-700/10 to-fuchsia-500/6 border border-indigo-500/20 rounded-xl p-5">
          ...
          <h2 className="text-2xl font-bold text-indigo-400 mt-1">{formatCurrency(shiftData.malam)}</h2>
        </div>
        ...
        <div className="bg-gray-800/60 border border-gray-700/40 rounded-xl p-5">
          ...
          <h2 className="text-2xl font-bold text-yellow-400 mt-1">{formatCurrency(shiftData.total)}</h2>
        </div>
      </div>
        ...
        {normalized.length > 0 ? (
          <div className="space-y-3 max-h-72 overflow-y-auto">
            {normalized.map((entry) => {
              ...
              return (
                <div key={entry.id || t.getTime()} className="flex justify-between items-center bg-gray-900/60 px-4 py-3 rounded-lg border border-gray-700/50 hover:bg-gray-900/80 transition-all">
                  <div>
                    <p className="text-sm font-medium text-white">{entry.room}</p>
                    <p className="text-xs text-gray-400">Kasir: {entry.handledBy || entry.cashier || "-" } • {formatCurrency(price)}</p>
                  </div>
