...
export default function SidebarForm({ saving = false }) {
  const { addBooking, bookings } = useFirebaseBookings();
  ...
  const handleAdd = async () => {
    ...
    if (totalMinutes <= 0) return alert("Durasi tidak valid.");
  ...
  return (
    <aside className="w-full md:w-80 lg:w-72 h-screen flex flex-col bg-gray-900 p-6 text-white shadow-xl fixed left-0 top-0 md:static overflow-y-auto">
